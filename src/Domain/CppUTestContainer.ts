    ...
    for (const executableGroup of this.suites.values()) {
      for (const testGroup of executableGroup.children) {
        for (const test of (testGroup as CppUTestGroup).children) {
          ...
          const testOutput = await runner.RunTest(testGroup.label, test.label);
          const testResult = this.resultParser.GetResult(testOutput);
    ...
    for (const executableGroup of this.suites.values()) {
      ...
      for (const test of testsToRun) {
        if (test && runner) {
          ...
          try {
            const testOutput = await runner.RunTest(test.group, test.label);
            const testResult = this.resultParser.GetResult(testOutput);
